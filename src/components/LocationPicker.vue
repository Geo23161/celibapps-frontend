Pi<template>
    <ion-modal mode="ios" :is-open="isOpen">
      <ion-header>
        <ion-toolbar>
          ...
          <ion-buttons v-if="quart.name != ''" slot="end">
            <ion-button color="primary" @click="done(quart)"
              >Valider</ion-button
            >
          </ion-buttons>
        </ion-toolbar>
      ...
      <ion-content class="ion-padding">
        <div style="color: #120c2b; padding-bottom: 1rem">
          Entrez et recherchez le nom des places et des lieux
        </div>
        <ion-searchbar
          @ionInput="(e) => (place = e.target.value as string)"
          placeholder="entrez..."
          @keyup.enter="get_quart()"
        ></ion-searchbar>
        ...
        <div v-if="place == ''">
          <div
            ...
            "
          >
            <img :src="'../../imgs/locat.svg'" style="width: 100%" />
          </div>
          <div style="text-align: center; padding: 0.7rem">
            Vous desirez rencontrer des personnes aux alentours de...
          </div>
        </div>
        ...
        <div v-if="quarts.length" style="padding-top: 1rem">
          <ion-item
            v-for="q in quarts"
            :key="`${q.name}:${q.lat}`"
            @click.stop="includes_quart(q)"
            button
          >
            <ion-checkbox 
              :checked="
                q.name == quart.name && q.lat == quart.lat && q.lng == quart.lng
              "
              slot="start"
            ></ion-checkbox>
        ...
        <div v-if="quart.name != ''" style="padding: 1rem">
          <ion-button @click="done(quart)" mode="md" expand="full"
            >Valider</ion-button
          >
        </div>
      </ion-content>
