        ...
        <ion-content :scroll-y="false">
            <div class="com_body">
                <div class="real_body">
                    ...
                    <div v-if="target">
                        <div v-for="p in [target]" :key="p.id" class="post">
                            <div>
                            ...
                            <div class="p_body">
                                ...
                                <div class="p_interac">
                                    ...
                                    <button @click="open_post(p)" class="pp_icon">
                                        <ion-icon :icon="p.i_comment ? chatbox : chatboxOutline" style="font-size: 5vw;" />
                                        <span style="font-size: 3vw; position: relative; bottom: 1.5vw;">{{ p.get_comments
                                        }}</span>
                                    </button>
                    ...
                    <div style="display: flex; justify-content: space-between; padding: 2vh 1.5vh; align-items: center;">
                        <button @click="router.back()" class="retou">
                    ...
                    <div id="containers" v-if="posts.length" style="overflow: scroll; position: relative; flex-grow: 1;">
                        <div v-for="p in posts" :key="p.id" class="post">
                            ...
                            <div class="p_body">
                                ...
                                <div v-if="p.get_image" class="h_txt">
                                    <img class="imgfull" :src="p.get_image" />
                                </div>
                                <div class="p_interac">
                                    <button @click="likes_fact(p)" class="pp_icon">
                                        <ion-icon :icon="p.i_like ? heart : heartOutline" style="font-size: 5vw;" />
                ...
                <div class="add_post" :class="{ hide_adding: !is_writing }">
                    <div class="r_post">
                        ...
                        <div @input="e => text = (e.target as any).innerText" class="my__in editable-div"
				...
				backgroundImage: `url('${image_url}')`
                        ...
                        <button @click="create_post()" class="send_aa is_norm" :class="{ is_upoo : text != '' || image, is_norm : !(text != '' || image) }" >
                            Envoyer
                        </button>
                    </div>
                </div>
...
import { IonPage, IonContent, IonSegment, IonSegmentButton, IonIcon, IonActionSheet, ActionSheetButton, onIonViewDidEnter } from "@ionic/vue"
import axios from "axios";
import { albums, chatbox, chatboxOutline, ellipseOutline, ellipsisVertical, ellipsisVerticalOutline, heartHalf, heartOutline, newspaper, settings, shareSocial, shareSocialOutline, heart, paperPlane, brush, pencil, close, add, open, exit, personRemove, arrowBack } from "ionicons/icons";
...
const get_posts = async () => {
    ...
    const resp = await axios.post('api/get_comments/' + p_id.value + '/', {
...
const set_likes = async (p: any) => {
    const resp = await axios.post('api/likes_fact/' + p.id + '/', {
        headers: {
            Authorization: `Bearer ${await access_tok(router, undefined)}`,
        },
    })
...
const create_post = async () => {
    ...
    form.append('origin', p_id.value.toString())
...
onIonViewDidEnter(() => {
    target.value = JSON.parse(localStorage.getItem("last:post") as string)
    p_id.value = parseInt(route.params.id as string)
})
